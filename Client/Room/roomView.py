...
class RoomView(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.lobby_button = ctk.CTkButton(
            self.room_frame,
            text="Go to lobby",
            command=self.lobby_button_clicked,
            height=40,
            width=300,
        )
        ...
        """
        self.room_treeview = ttk.Treeview(
            master=self.room_frame, columns=("", "Player", "Ready"), padding=25
        )

        self.room_treeview.heading(
            column="#1",
            text="Index",
            anchor="center",
            command=lambda i: self.sort_by("player_id"),
        )
        ...
        self.room_treeview.heading(
            ...
            text="Player",
            anchor="center",
            command=lambda i=0: self.sort_by("username"),
        )
        ...
        self.room_treeview.heading(
            ...
            text="Ready",
            anchor="center",
            command=lambda i=1: self.sort_by("ready"),
        )

        self.room_treeview.column(column="#0", width=0, anchor="center")
        self.room_treeview.column(column="#1", width=180, anchor="center")
        self.room_treeview.column(column="Player", width=250, anchor="center")
        self.room_treeview.column(column="Ready", width=250, anchor="center")
    ...
    def owner_view(self, is_owner):
        if is_owner:
            self.owner_button = ctk.CTkButton(
                self.room_frame,
                text="Start game",
            ...
            self.state_button.grid(column=1, row=0, pady=(10, 10))
    ...
    def draw_room(self, lst):
        if self.updating is False:
            ...
            for player_index, item in enumerate(lst):
                ...
                self.room_treeview.insert(
                    "", "end", values=(player_index + 1,) + values
                )
