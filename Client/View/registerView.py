...
class RegisterView(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.label = ctk.CTkLabel(self.register_frame, text='Signup', font=('Times New Roman', 20, "bold"))
        self.label.grid(row=0, column=1, pady=15, padx=25)

        # Username entry
        self.username_entry = ctk.CTkEntry(self.register_frame, font=('Courier New', 15),
                                           width=250, placeholder_text="Username")
        self.username_entry.grid(row=1, column=1, sticky=tk.NSEW, padx=25, pady=10, ipady=5)

        # Email entry
        self.email_entry = ctk.CTkEntry(self.register_frame, font=('Courier New', 15),
                                        width=250, placeholder_text="Email")
        self.email_entry.grid(row=2, column=1, sticky=tk.NSEW, padx=25, pady=10, ipady=5)

        # password entry
        self.password_entry = ctk.CTkEntry(self.register_frame, font=('Courier New', 15),
                                           width=250, placeholder_text="Password", show="*")
        self.password_entry.grid(row=3, column=1, sticky=tk.NSEW, padx=25, pady=10, ipady=5)

        # password rentry
        self.password_reentry = ctk.CTkEntry(self.register_frame, font=('Courier New', 15),
                                           width=250, placeholder_text="Confirm Password", show="*")
        self.password_reentry.grid(row=5, column=1, sticky=tk.NSEW, padx=25, pady=10, ipady=5)

        # register button
        self.signup_button = ctk.CTkButton(self.register_frame, text='Signup', command=self.register_button_clicked, height=40)
        self.signup_button.grid(row=7, column=1, columnspan=2, sticky=tk.NSEW, padx=25, pady=10)

        # register button
        self.login_button = ctk.CTkLabel(self.register_frame, text='Already have an account? Login', height=40)
